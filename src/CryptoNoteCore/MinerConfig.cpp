...
const command_line::arg_descriptor<uint32_t>    arg_stake_amount    = {"stake", "Specify wanted stake amount", 0, true};
}
...
void MinerConfig::init(const boost::program_options::variables_map& options) {
  ...
  if (command_line::has_arg(options, arg_stake_amount)) {
    stakeAmount = command_line::get_arg(options, arg_stake_amount);
  }
}
