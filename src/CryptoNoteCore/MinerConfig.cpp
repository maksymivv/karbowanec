...
const command_line::arg_descriptor<std::string> arg_stake_amount    = {"stake", "Specify wanted stake amount", "", true};
}
...
void MinerConfig::initOptions(boost::program_options::options_description& desc) {
  ...
  command_line::add_arg(desc, arg_stake_amount);
}
...
void MinerConfig::init(const boost::program_options::variables_map& options) {
  ...
  if (command_line::has_arg(options, arg_stake_amount)) {
    Common::Format::parseAmount(command_line::get_arg(options, arg_stake_amount), stakeAmount);
  }
}
