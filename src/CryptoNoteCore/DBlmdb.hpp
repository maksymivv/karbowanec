...
public:
	explicit DBlmdb(Common::OpenMode open_mode, const std::string &full_path,
	...
	void put(const std::string &key, const CryptoNote::BinaryArray &value, bool nooverwrite);
	void put(const std::string &key, const std::string &value, bool nooverwrite);

	bool get(const std::string &key, CryptoNote::BinaryArray &value) const;
    ...
    CryptoNote::BinaryArray get_value_array() const;
