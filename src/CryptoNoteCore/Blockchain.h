...
namespace CryptoNote {
  ...
  public:
    ...
    uint64_t blockCumulativeStake(size_t i);
    uint64_t blockActualStake(size_t i);
  ...
  private:
    ...
    typedef parallel_flat_hash_map<uint64_t, std::vector<std::pair<Crypto::Hash, uint64_t>>> locked_amounts_container; // unlock_time (height), Crypto::Hash - block hash and amount
    ...
    bool validate_miner_transaction(const Block& b, uint32_t height, size_t cumulativeBlockSize, uint64_t alreadyGeneratedCoins, uint64_t fee, uint64_t& reward, int64_t& emissionChange, uint64_t& baseStake, uint64_t& transactionStake);
