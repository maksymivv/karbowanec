...
bool Blockchain::prevalidate_miner_transaction(const Block& b, uint32_t height) {
  ...
  if (extraSize > CryptoNote::parameters::MAX_EXTRA_SIZE) {
    logger(ERROR, BRIGHT_RED)
...
bool Blockchain::validate_miner_transaction(const Block& b, uint32_t height, size_t cumulativeBlockSize,
  ...
  if (!m_currency.getBlockReward(blockMajorVersion, height, blocksSizeMedian, cumulativeBlockSize, alreadyGeneratedCoins, fee, reward, emissionChange)) {
