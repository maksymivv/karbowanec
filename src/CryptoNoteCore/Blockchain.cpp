...
bool Blockchain::getBlockLongHash(Crypto::cn_context &context, const Block& b, Crypto::Hash& res) {
  ...
  uint32_t  maxHeight = std::min<uint32_t>(boost::get<BaseInput>(b.baseTransaction.inputs[0]).blockIndex - 1 - m_currency.minedMoneyUnlockWindow_v1(), static_cast<uint32_t>(m_blocks.size() - 1));

  // Phase 1: Hashing the current blockdata
  cn_fast_hash(bd.data(), bd.size(), hash_1);

  // Phase 2: Blockchain Specific Operations
  // Get the corresponding 8 blocks from blockchain based on phase 1
  // and throw them into the pot
  // stir the pot - iterate it

  hash_2 = hash_1;

  for (uint32_t it = 0; it < ITER; it++) {
    pot.clear();
    // throw our current block into pot p
    pot.insert(std::end(pot), std::begin(bd), std::end(bd));
    for (auto i = 0; i < 8; ++i) {     
      uint32_t chunk = *reinterpret_cast<uint32_t *>(&hash_2.data[i * 4 - 4]);
      uint32_t height_i = chunk % maxHeight;
    ...
    cn_fast_hash(pot.data(), pot.size(), hash_2);
  }
  ...
  // add grain of species and salt, which is final hash_2
  // to final state of the pot after iterations
  Crypto::argon2d_hash(pot.data(), pot.size(), hash_2.data, sizeof(hash_2), m_cost, lanes, t_cost, hash_3);
