...
CryptoNote::BinaryArray DBlmdb::Cursor::get_value_array() const {
	return CryptoNote::BinaryArray(data.data(), data.data() + data.size());
}
...
void DBlmdb::put(const std::string &key, const CryptoNote::BinaryArray &value, bool nooverwrite) {
...
bool DBlmdb::get(const std::string &key, CryptoNote::BinaryArray &value) const {
	lmdb::Val val1;
...
void DBlmdb::run_tests() {
	delete_db("temp_db");
	{
		DBlmdb db(Common::O_CREATE_NEW, "temp_db");
