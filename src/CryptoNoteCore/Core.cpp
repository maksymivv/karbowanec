...
bool core::get_block_template(Block& b, const AccountPublicAddress& adr, difficulty_type& diffic, uint32_t& height, const BinaryArray& ex_nonce) {
  ...
  difficulty_type currentAvgDifficulty = m_blockchain.getAvgDifficulty(height - 1, m_currency.expectedNumberOfBlocksPerDay());

  // After block v 5 simplify miner tx generation and reward calc.
...
bool core::getBlockReward(uint32_t height, uint8_t blockMajorVersion, size_t medianSize, size_t currentBlockSize, uint64_t alreadyGeneratedCoins, uint64_t fee,
  ...
  difficulty_type currentAvgDifficulty = m_blockchain.getAvgDifficulty(height, m_currency.expectedNumberOfBlocksPerDay());
