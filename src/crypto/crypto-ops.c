...
void ge_double_scalarmult_base_vartime3(ge_p3 *rr, const unsigned char *a, const ge_p3 *A, const unsigned char *b) {
  signed char aslide[256];
  signed char bslide[256];
  ge_dsmp Ai; /* A, 3A, 5A, 7A, 9A, 11A, 13A, 15A */
  ge_p1p1 t;
  ...
  ge_p2_0(&r);
  ge_p3_0(rr); // We will not enter "for" below for some inputs
...
/* Assumes that a[31] <= 127 */
void ge_scalarmult3(ge_p3 *rr, const unsigned char *a, const ge_p3 *A) {
  signed char e[64];
  int carry, carry2, i;
  ge_cached Ai[8]; /* 1 * A, 2 * A, ..., 8 * A */
  ge_p1p1 t;
...
void ge_p3_mul8(ge_p1p1 *r, const ge_p3 *t) {
  ge_p2 u;
  ge_p3_dbl(r, t);
  ge_p1p1_to_p2(&u, r);
  ge_p2_dbl(r, &u);
