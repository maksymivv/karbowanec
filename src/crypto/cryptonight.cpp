...
void cn_slow_hash_gpu(cn_context &context, const void *data, size_t length, Hash &hash) {
  if (hw_check_aes())
    cryptonight_hash_gpu<true, CRYPTONIGHT_GPU>(data, length, reinterpret_cast<char *>(&hash), context);
  else
    cryptonight_hash_gpu<false, CRYPTONIGHT_GPU>(data, length, reinterpret_cast<char *>(&hash), context);
}
