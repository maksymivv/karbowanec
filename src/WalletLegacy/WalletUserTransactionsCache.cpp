...
std::deque<std::shared_ptr<WalletLegacyEvent>> WalletUserTransactionsCache::onTransactionUpdated(const TransactionInformation& txInfo, int64_t txBalance, 
	const std::vector<TransactionOutputInformation>& newDepositOutputs,
	const std::vector<TransactionOutputInformation>& spentDepositOutputs, const Currency& currency) {
...
std::deque<std::shared_ptr<WalletLegacyEvent>> WalletUserTransactionsCache::onTransactionDeleted(const Hash& transactionHash) {
  TransactionId id = CryptoNote::WALLET_LEGACY_INVALID_TRANSACTION_ID;
  ...
  if (id != CryptoNote::WALLET_LEGACY_INVALID_TRANSACTION_ID) {
	...
	events.push_back(std::shared_ptr<WalletLegacyEvent>(new WalletTransactionUpdatedEvent(id)));
    ...
    if (depositIdSequenceStart != depositIdSequenceEnd || !unspentDeposits.empty()) {
	  ...
	  events.push_back(std::shared_ptr<WalletLegacyEvent>(new WalletDepositsUpdatedEvent(std::move(unspentDeposits))));
    }
  } else {
