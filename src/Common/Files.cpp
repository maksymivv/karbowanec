...
uint64_t FileStream::seek(uint64_t pos, int whence) {
	...
	if (!SetFilePointerEx(handle, lpos, &rpos, whence))
		throw std::runtime_error("Error seeking file in seek, GetLastError=" + Common::to_string(GetLastError()));
...
size_t FileStream::writeSome(const void *data, size_t size) {
	...
	if (!WriteFile(handle, data, static_cast<DWORD>(size), &si, nullptr))
		throw std::runtime_error("Error writing file, GetLastError()=" + Common::to_string(GetLastError()));
	return si;
#else
...
size_t FileStream::readSome(void *data, size_t size) {
	...
	if (!ReadFile(handle, data, static_cast<DWORD>(size), &si, nullptr))
		throw std::runtime_error("Error reading file, GetLastError()=" + Common::to_string(GetLastError()));
	return si;
#else
