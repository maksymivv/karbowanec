...
pool_wallet::pool_wallet(System::Dispatcher& dispatcher, const CryptoNote::Currency& currency, Logging::LoggerManager& log) :
  ...
  m_consoleHandler.setHandler("create_integrated", boost::bind(&pool_wallet::create_integrated, this, _1), "create_integrated <payment_id> - Create an integrated address with a payment ID");
  m_consoleHandler.setHandler("export_keys", boost::bind(&pool_wallet::export_keys, this, _1), "Show the secret keys of the current wallet");
  ...
  m_consoleHandler.setHandler("list_transfers", boost::bind(&pool_wallet::listTransfers, this, _1), "list_transfers <height> - Show all known transfers from a certain (optional) block height");
  ...
  m_consoleHandler.setHandler("outputs", boost::bind(&pool_wallet::show_num_unlocked_outputs, this, _1), "Show the number of unlocked outputs available for a transaction");
  m_consoleHandler.setHandler("optimize", boost::bind(&pool_wallet::optimize_outputs, this, _1), "Combine many available outputs into a few by sending a transaction to self");
...
bool pool_wallet::create_integrated(const std::vector<std::string>& args/* = std::vector<std::string>()*/) 
{
...
bool pool_wallet::export_keys(const std::vector<std::string>& args/* = std::vector<std::string>()*/) {
  AccountKeys keys;
  m_wallet->getAccountKeys(keys);
...
//----------------------------------------------------------------------------------------------------
bool pool_wallet::show_num_unlocked_outputs(const std::vector<std::string>& args) {
  try {
    size_t num_unlocked_outputs = m_wallet->getUnlockedOutputsCount();
...
//----------------------------------------------------------------------------------------------------
bool pool_wallet::optimize_outputs(const std::vector<std::string>& args) {
  try {
    ...
    uint64_t ttl = 0;
    CryptoNote::TransactionId tx = m_wallet->sendTransaction(transfers, fee, extraString, mixIn, unlockTimestamp);
...
//----------------------------------------------------------------------------------------------------
std::string pool_wallet::resolveAlias(const std::string& aliasUrl) {
  std::string host;
  std::string uri;
...
int main(int argc, char* argv[]) {
  ...
  Logging::LoggerRef logger(m_logManager, "simplewallet");
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    if (command_line::get_arg(vm, command_line::arg_help)) {
      CryptoNote::Currency tmp_currency = CryptoNote::CurrencyBuilder(m_logManager).currency();
      CryptoNote::pool_wallet tmp_wallet(dispatcher, tmp_currency, m_logManager);
  ...
  m_logManager.configure(buildLoggerConfiguration(logLevel, Common::ReplaceExtenstion(argv[0], ".log")));

  logger(INFO, BRIGHT_WHITE) << CRYPTONOTE_NAME << " wallet v" << PROJECT_VERSION_LONG;

  CryptoNote::Currency currency = CryptoNote::CurrencyBuilder(m_logManager).
  ...
  if (command_line::has_arg(vm, Tools::pool_rpc_server::arg_rpc_bind_port)) {
    ...
    std::unique_ptr<IWalletLegacy> wallet(new WalletLegacy(currency, *node.get(), m_logManager));
    ...
    Tools::pool_rpc_server wrpc(dispatcher, m_logManager, *wallet, *node, currency, walletFileName);
    ...
    CryptoNote::pool_wallet wal(dispatcher, currency, m_logManager);
