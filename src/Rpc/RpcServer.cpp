...
bool RpcServer::on_get_info(const COMMAND_RPC_GET_INFO::request& req, COMMAND_RPC_GET_INFO::response& res) {
  ...
  res.next_reward = m_core.currency().calculateReward(res.already_generated_coins);
  res.base_stake = m_core.getBaseStake();
  uint64_t totalLocked = 0;
  if (res.block_major_version >= CryptoNote::BLOCK_MAJOR_VERSION_5) {
    totalLocked = m_core.getLockedAmount(res.height - 1);
  }
...
bool RpcServer::on_start_mining(const COMMAND_RPC_START_MINING::request& req, COMMAND_RPC_START_MINING::response& res) {
  ...
  if (!m_core.get_miner().start(adr, static_cast<size_t>(req.threads_count), req.stake_amount)) {
...
bool RpcServer::on_prepareblocktemplate(const COMMAND_RPC_PREPARE_BLOCKTEMPLATE::request& req, COMMAND_RPC_PREPARE_BLOCKTEMPLATE::response& res) {
  if (req.reserve_size > TX_EXTRA_NONCE_MAX_COUNT) {
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_TOO_BIG_RESERVE_SIZE, "To big reserved size, maximum 255" };
  }
